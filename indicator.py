...
def ffr_function(input_data):
    numpy_value,pyramid_table,idx2=basic_function(input_data)
    numpy_value, PROD_data, SALES_data=random_function()
    ...
    FFRFDR_1Y=pd.concat([FDR_1Y,FFR_1Y],axis=1).dropna()
    FFRFDR_1Y.index=range(1,13)
    FFRFDR_2Y=pd.concat([FDR_2Y,FFR_2Y],axis=1).dropna()
    FFRFDR_2Y.index=range(1,13)
    FFRFDR_3Y=pd.concat([FDR_3Y,FFR_3Y],axis=1).dropna()
    FFRFDR_3Y.index=range(1,13)
    FFRFDR_4Y=pd.concat([FDR_4Y,FFR_4Y],axis=1).dropna()
    FFRFDR_4Y.index=range(1,13)
    FRFDR_Total=pd.concat([FFRFDR_1Y,FFRFDR_2Y,FFRFDR_3Y,FFRFDR_4Y],axis=1)
